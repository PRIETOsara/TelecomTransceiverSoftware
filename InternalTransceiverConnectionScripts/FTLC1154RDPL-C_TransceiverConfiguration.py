...
class FinisarManager:
    ...
    def set_rx_rate_select(self, msb, lsb):
        """
        Sets RX Rate Select bits using separate MSB and LSB lists.
        Note: Operates on Lower Page 00h, Register 87 (0x57).
        
        Bit mapping:
        Lane 3: Bit 7 (MSB), Bit 6 (LSB)
        Lane 2: Bit 5 (MSB), Bit 4 (LSB)
        Lane 1: Bit 3 (MSB), Bit 2 (LSB)
        Lane 0: Bit 1 (MSB), Bit 0 (LSB)
        """
        val = 0
        val |= ((msb[3] & 1) << 7) | ((lsb[3] & 1) << 6)
        val |= ((msb[2] & 1) << 5) | ((lsb[2] & 1) << 4)
        val |= ((msb[1] & 1) << 3) | ((lsb[1] & 1) << 2)
        val |= ((msb[0] & 1) << 1) | ((lsb[0] & 1) << 0)

        cmd = [0x08, 0x00, 0xda, 0xa0, 0x00, 0x00, 0x00, 0x00, 
               0x08, 0x00, 0x9a, 0x57, 0x00, 0x00, 0x00, 0x00, 
               0x08, 0x00, 0xba, val,  0x00, 0x00, 0x00, 0x00, 
               0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
        return self._send_command(cmd)

    def set_tx_rate_select(self, msb, lsb):
        """
        Sets TX Rate Select bits using separate MSB and LSB lists.
        Note: Operates on Lower Page 00h, Register 88 (0x58).
        
        Bit mapping matches RX (Lane 3 down to Lane 0).
        """
        val = 0
        val |= ((msb[3] & 1) << 7) | ((lsb[3] & 1) << 6)
        val |= ((msb[2] & 1) << 5) | ((lsb[2] & 1) << 4)
        ...
        cmd = [0x08, 0x00, 0xda, 0xa0, 0x00, 0x00, 0x00, 0x00, 
               0x08, 0x00, 0x9a, 0x58, 0x00, 0x00, 0x00, 0x00, 
               0x08, 0x00, 0xba, val,  0x00, 0x00, 0x00, 0x00, 
               0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
        return self._send_command(cmd)
...
def main():
    ...
    # Example 4: Set separated MSB/LSB Rate Selection on Page 00
    fm.set_page(0)
    ack_rx_rs = fm.set_rx_rate_select(RX_RS_MSB, RX_RS_LSB)
    print(f"RX Rate Select Configured. Ack: {ack_rx_rs}")

    ack_tx_rs = fm.set_tx_rate_select(TX_RS_MSB, TX_RS_LSB)
    print(f"TX Rate Select Configured. Ack: {ack_tx_rs}")
